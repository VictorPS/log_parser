RSpec.describe PageVisitListDecorator do
  subject { PageVisitListDecorator.new(page_visit_list) }
  let(:page_visit1) { PageVisit.new(webpage: '/home', device_ip: '184.123.665.067') }
  let(:page_visit2) { page_visit1.dup }
  let(:page_visit3) { PageVisit.new(webpage: '/help_page', device_ip: '126.318.035.038') }
  let(:page_visit4) { PageVisit.new(webpage: '/home', device_ip: '126.318.035.038') }
  let(:page_visit_list) { PageVisitList.new([page_visit1, page_visit2, page_visit3, page_visit4]) }

  describe '#asc_page_visit_count' do
    it 'returns a formatted list of the visited pages ordered by most visited' do
      expect(subject.asc_page_visit_count).to eq(
        [
          '/home 3 visits',
          '/help_page 1 visit'
        ]
      )
    end
  end
end
